...
export const getTransactionByIdService = async (
  ...
  if (!transaction) {
    throw new NotFoundException("Transaction not foudn");
  }

  return transaction;
};

export const duplilcateTransactionService = async (
  userId: string,
  transactionId: string
) => {
  const transaction = await TransactionModel.findOne({
    ...
    const bulkOps = transactions.map((tx) => ({
      insertOne: {
        document: {
          ...
          lastProcessed: null, // changed naming
          createdAt: new Date(),
          updatedAt: new Date(),
        },
      },
...
export const scanReceiptService = async (
  ...
  try {
    ...
    const base64String = Buffer.from(responseData.data as ArrayBuffer).toString(
      "base64"
    );
